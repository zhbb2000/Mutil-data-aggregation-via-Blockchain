...
class CurveFp:
    ...
    def generate_random_point(self):
        """Generates a random point on the curve that is not a multiple of the base point G."""
        while True:
            x = RandomInteger.between(1, self.P - 1)
            y_squared = (pow(x, 3, self.P) + self.A * x + self.B) % self.P
            y = Math.modularSquareRoot(y_squared, self.P)
            if y is not None:  # If y has a square root
                # Randomly choose one of the two possible y values
                if RandomInteger.between(0, 1) == 0:
                    y = self.P - y
                point = Point(x, y)
