...
for num in range(10):
    for n in participant_counts:
        ...
        i = int(n/100 -1)
        BV_FN[i] += end_time_mul_verify - start_time_mul_verify
        ...
        aggretime_FN[i] += end_time_CT_aggregation - start_time_CT_aggregation
        ...
        data = [signature.s, signature.r.x, signature.r.y, signature.m_hash, publicKey.point.x, publicKey.point.y,
                publicKey.curve.N]
        deployed_Contract.functions.UploadUser_ECDSA("FN", data).transact({'from': w3.eth.accounts[0]})
        end_time_sign = time.time()
        sign_uplord[i] += end_time_sign - start_time_sign
