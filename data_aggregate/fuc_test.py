...
upper_data_sets = [[70, 100], [70, 100, 200, 50], [70, 100, 200, 50, 120, 110], [70, 100, 200, 50, 120, 110, 40, 60]]
...
for upper_data in upper_data_sets:
    ...
    for n in participant_counts:
        ...
        print(sys.getsizeof(str(DP_IDs[0])))
        ...
        print(sys.getsizeof(int(Vector_seq[0])))
        ...
        for i in range(len(ct_i_seq)):
            ...
            if i == 0:
                print("commit size", sys.getsizeof(C["commit"].x))
        ...
        print(f"Time taken of sign for {n} participants:{sign_time / n} seconds per participant")
        print("signature size:", sys.getsizeof(mul_signature[0]))
        while n == 100:
            DP_time_ecos.append((end_time_encryption - start_time_encryption) / n)
            DP_time_ccos.append(perdersen_cost / n)
            DP_time_scos.append(sign_time / n)
            break
        ...
        print(f"Time taken of multiple ECDSA verifications for {n} participants：{end_time_mul_verify - start_time_mul_verify} seconds")
        # Test homomorphism of Paillier encryption
